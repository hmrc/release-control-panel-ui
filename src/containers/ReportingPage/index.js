...
export default class Reporting extends BaseComponent {
    ...
    componentDidMount()
    {
        ...
        d3.json(config.apiURL + "/statistics", (error, data) =>
        {
            if (!this.m_isMounted)
                return;
            ...
            {
                let versions = data.filter(p => p.projectName == project.name && p.environment == environments.Production);

                //let versions = data[project.name][environments.Production];
                let reportData = versions.map(item => [item.version, (item.lifeTime / 1000 / 60 / 60 / 24).toFixed(2)]);
                ...
                let reportDataDays = versions.map(item => item.lifeTime / 1000 / 60 / 60 / 24);
                                    ...
                                    <div key={index}>
                                        ...
                                        <div className="form-horizontal">
                                            <div className="form-group">
                                                <label htmlFor={`chart-search-${project.name}`}
                                                       className="col-sm-2 control-label">Search</label>
                                                <div className="col-sm-10">
                                                    <TextField
                                                        id={`chart-search-${project.name}`}
                                                        onChange={this.handleSearchVersionChange.bind(this, project)}
                                                        style={{ width: "100%" }}/>
                                                </div>
                                            </div>
                                        </div>
